...
COMMAND="cd /scratch/imos-students/ksteiner/BeamElastoDynamics && python run_passive_train.py --volume-loss-weight=0.001 --epochs=500"

runai submit \
  --name train-volume-tr0-001-500 \
  --image ${IMAGE_NAME} \
  --gpu 1 \
  --run-as-uid ${LDAP_UID} \
  --run-as-gid ${LDAP_GID} \
